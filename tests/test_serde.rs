...
{
    let serialized = serde_norway::to_string(&thing).unwrap();
    assert_eq!(yaml, serialized);

    let value = serde_norway::to_value(thing).unwrap();
    let serialized = serde_norway::to_string(&value).unwrap();
    assert_eq!(yaml, serialized);
...
fn test_float() {
    ...
    let float: f64 = serde_norway::from_str(indoc! {"
        .nan
    "})
    .unwrap();
    assert!(float.is_nan());
...
fn test_float32() {
    ...
    let single_float: f32 = serde_norway::from_str(indoc! {"
        .nan
    "})
    .unwrap();
...
fn test_char() {
    ...
    let yaml = indoc! {"
        '.'
    "};
    assert_eq!(yaml, serde_norway::to_string(&ch).unwrap());
...
fn test_map_key_value() {
    ...
    let yaml = indoc! {"
        k: v
    "};
    assert_eq!(yaml, serde_norway::to_string(&Map).unwrap());
}
