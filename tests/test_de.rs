...
use serde_norway::{Deserializer, Number, Value};
...
{
    let deserialized: T = serde_norway::from_str(yaml).unwrap();
    ...
    let deserialized: T = serde_norway::from_value(value).unwrap();
    assert_eq!(*expected, deserialized);

    serde_norway::from_str::<serde::de::IgnoredAny>(yaml).unwrap();
...
fn test_i128_big() {
    ...
    let yaml = indoc! {"
        -9223372036854775809
    "};
    assert_eq!(expected, serde_norway::from_str::<i128>(yaml).unwrap());

    let octal = indoc! {"
        -0o1000000000000000000001
    "};
    assert_eq!(expected, serde_norway::from_str::<i128>(octal).unwrap());
}
...
fn test_u128_big() {
    ...
    "};
    assert_eq!(expected, serde_norway::from_str::<u128>(yaml).unwrap());

    let octal = indoc! {"
        0o2000000000000000000000
    "};
    assert_eq!(expected, serde_norway::from_str::<u128>(octal).unwrap());
}
...
fn test_de_mapping() {
    ...
    struct Data {
        pub substructure: serde_norway::Mapping,
    }
    let yaml = indoc! {"
        substructure:
          a: 'foo'
    ...
    let mut expected = Data {
        substructure: serde_norway::Mapping::new(),
    };
    expected.substructure.insert(
        serde_norway::Value::String("a".to_owned()),
        serde_norway::Value::String("foo".to_owned()),
    );
    expected.substructure.insert(
        serde_norway::Value::String("b".to_owned()),
        serde_norway::Value::String("bar".to_owned()),
    );
...
fn test_bomb() {
    ...
    assert_eq!(expected, serde_norway::from_str::<Data>(yaml).unwrap());
}
...
fn test_numbers() {
    ...
    for &(yaml, expected) in &cases {
        let value = serde_norway::from_str::<Value>(yaml).unwrap();
        match value {
...
fn test_nan() {
    ...
    assert!(serde_norway::from_str::<f32>(".nan")
        .unwrap()
        .is_sign_positive());
    assert!(serde_norway::from_str::<f64>(".nan")
        .unwrap()
        .is_sign_positive());
}
...
fn test_no_required_fields() {
    ...
    for document in ["", "# comment\n"] {
        ...
        let deserialized: NoRequiredFields = serde_norway::from_str(document).unwrap();
        assert_eq!(expected, deserialized);

        let expected = Vec::<String>::new();
        let deserialized: Vec<String> = serde_norway::from_str(document).unwrap();
        assert_eq!(expected, deserialized);

        let expected = BTreeMap::new();
        let deserialized: BTreeMap<char, usize> = serde_norway::from_str(document).unwrap();
        assert_eq!(expected, deserialized);

        let expected = None;
        let deserialized: Option<String> = serde_norway::from_str(document).unwrap();
        assert_eq!(expected, deserialized);

        let expected = Value::Null;
        let deserialized: Value = serde_norway::from_str(document).unwrap();
        assert_eq!(expected, deserialized);
    }
}
...
fn test_empty_scalar() {
    ...
    let yaml = "thing:\n";
    let expected = Struct {
        thing: serde_norway::Sequence::new(),
    };
    test_de(yaml, &expected);

    let expected = Struct {
        thing: serde_norway::Mapping::new(),
    };
    test_de(yaml, &expected);
}
